...
int main(int argc, char** argv)
{
  ...
  */
   ...
   */
   ...
   printf("Connected to Arduino, about to start looping\n");
   while(ros::ok())
   {
      printf("In loop\n");
      ...
      printf("Received X:%s", buf); 
      
      // Get the Y coordinate
      write(fd, "y", 1);
      res = read(fd, buf, sizeof(buf)/sizeof(buf[0]));
      buf[res] = 0; // set end of string so we can printf
      printf("Received Y:%s", buf); 

      // Get the X coordinate
      write(fd, "f", 1);
      res = read(fd, buf, sizeof(buf)/sizeof(buf[0]));
      buf[res] = 0; // set end of string so we can printf
      printf("Received F:%s", buf); 
   }
